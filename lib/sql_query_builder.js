...
p.set_fields = function (object) {
	var fields = this.fields;
	Object.keys(object).forEach(function (field_name) {
		fields[field_name] = object[field_name];
	});
	return this;
};

var FUNCS = {
	$distinct : function (field) { return "DISTINCT(`" + field + "`)"; }
};

p.set_select = function (object) {
	var fields = [];
...
p.where = function (wheres) {
	...
	if (wheres.$groups) {
		wheres.$groups.forEach(function (group) {
			this.groups.push(new WhereNode(group, this.table));
		}, this);
	...
	if (wheres.$or_groups) {
		wheres.$or_groups.forEach(function (group) {
			this.or_groups.push(new WhereNode(group, this.table));
		}, this);
	...
	if (Object.keys(wheres).length > 0) {
		this.groups.unshift(new WhereNode(wheres, this.table));
	}
...
p.build_select_query = function () {
	...
	if (this.select.indexOf(",") >= 0) {
		query.total = "SELECT COUNT(*) AS `total` FROM " + this.table;
	} else {
		query.total = "SELECT COUNT(" + this.select + ") AS `total` FROM " + this.table;
	}
