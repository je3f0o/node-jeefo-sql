...
function next (err, results, last_query, callback) {
	if (err) {
		console.error("jeefo-sql ERROR :", last_query);
		throw new Error(err);
	} else if (callback) {
		callback(null, results, last_query);
	}
}
...
function JeefoDB (config, use_pool_cache) {
