	...
	async.waterfall(
		[
			...
			function (records, fields, cb) {
				...
				if (records.length > 0) {
					$this.db.query($this.last_query.total, $this.last_query.values, cb);
				} else {
					cb(null, [], null);
				}
			},
	...
	async.waterfall(
		[
			...
			function (cb) {
				...
				qb.reset().where(wheres).limit(1);
				$this.last_query = qb.build_select_query();
	...
	async.waterfall(
		[
			...
			function (cb) {
				...
				qb.reset().where(wheres).bind(fields);
